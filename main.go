...
import (
	...
	"fmt"
	"os"
	"github.com/pavulla-tech/kampfumo/mua"
)
...
func main() {
	...
	encryptionClient := mua.NewEncriptionClient(privateKey, &privateKey.PublicKey)
	...
	sessionKey, err := encryptionClient.DecryptSessionKey(encryptedSessionKey)
	if err != nil {
		panic(fmt.Sprintf("Falha ao descriptar chave de sessão: %v", err))
	}

	decryptedData, err := encryptionClient.DecryptDataForUsage(encryptedData, nonce, sessionKey)
	if err != nil {
